"""
Test program for inlining phase
Integrates all six phases: lexer, parser, scope analyzer, type checker, code generator, and inliner
"""
...
def test_inlining():
    """Test program with inlining through all six phases"""
    
    # Test program with function and procedure calls
    sample_code = """
    glob { result }
    proc {
        display ( value ) {
    ...
    print("=" * 80)
    print("SPL COMPILER - SIX PHASE TEST (WITH INLINING)")
    print("=" * 80)
    print("\nSample Program:")
    print(sample_code)
    print("=" * 80)
    ...
    try:
        intermediate_code = generate_intermediate_code(parse_tree, analyzer.symbol_table, "intermediate_code.txt")
    ...
    try:
        inlined_code = inline_intermediate_code(parse_tree, analyzer.symbol_table, intermediate_code, "inlined_code.txt")
        ...
        if inlined_code:
            ...
            print(inlined_code)
            print("-" * 50)
            print(f"Inlined code saved to: inlined_code.txt")
        else:
